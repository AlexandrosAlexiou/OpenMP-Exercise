...
int main()
{
    ...
    #pragma omp parallel for num_threads(NUM_THREADS) private(i,j,k) private(sum) schedule(static,CHUNK_SIZE)
    for (i = 0; i < N; i++){
        for (j = 0; j < N; j++)
        {
          sum = 0;
    ...
    /* Save result in "Cmat1024_outer"
     */
    writemat("Cmat1024_outer", (int *) C, N);
    return (0);
}
